...
*   (2025-05-26): Implemented `coverage_planner_mode` parameter in `planner_params.yaml` and `helper_functions.cpp` to dynamically select input trajectory files (`dfki_pipe_fc.txt` or `dfki_pipe_helix.txt`).
...
*   Current development phase: Parameterization and configuration.
*   (As of 2025-05-26): `coverage_planner_mode` parameter successfully added and integrated into `helper_functions.cpp` for conditional file loading.
*   (2025-04-06): Completed extraction and processing of `run3` data (`ta_planner_good`, `fc_planner`) into CSV format. Data is organized in `results/run3/` subdirectories.
